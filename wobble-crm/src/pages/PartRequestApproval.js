...
export default function PartRequestApproval() {
  ...
  const handleApprove = async (id, caseId) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'cases', caseId), { jobStatus: 'Part Dispatch Pending' });
  ...
  return (
    <div>
      <div className="mb-6"><h2 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Part Request Approvals</h2><p className="text-gray-500 mt-1">Review, approve with PO, or reject part requests</p></div>
      {requests.length === 0 ? (
        <div className="card text-center py-12"><div className="text-6xl mb-4">✅</div><p className="text-gray-500">No pending part requests</p></div>
      ) : (
          ...
          {requests.map((req) => (
            <div key={req.id} className="card">
              <div className="flex flex-wrap justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-2"><span className="badge-yellow flex items-center gap-1"><FiClock /> Pending</span><span className="font-mono text-sm bg-gray-100 px-2 py-1 rounded">{req.jobId}</span></div>
                  <h3 className="text-xl font-semibold">{req.partName}</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-2 text-sm">
                    <div><p className="text-gray-500 text-xs">Quantity</p><p>{req.quantity}</p></div>
                    <div><p className="text-gray-500 text-xs">Variant</p><p>{req.variant || 'N/A'}</p></div>
                    <div><p className="text-gray-500 text-xs">Customer</p><p>{req.customerName}</p></div>
                    <div><p className="text-gray-500 text-xs">Request Date</p><p>{new Date(req.requestDate).toLocaleDateString()}</p></div>
                  </div>
                  {req.issueDescription && <div className="mt-2 p-2 bg-gray-100 rounded-lg text-sm"><p className="text-gray-500 text-xs">Reason</p><p>{req.issueDescription}</p></div>}
                </div>
