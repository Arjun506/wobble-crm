...
export default function PartRequestApproval() {
    ...
    return (
        <div>
            <div className="mb-6">
                <h2 className="text-3xl font-bold bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent">Part Request Approvals</h2>
                <p className="text-slate-400 mt-1">Review, approve with PO, or reject part requests</p>
            </div>

            {requests.length === 0 ? (
                <div className="card text-center py-12">
                    <div className="text-6xl mb-4">✅</div>
                    <p className="text-slate-400">No pending part requests</p>
                </div>
            ) : (
                            ...
                            <div className="flex flex-wrap justify-between gap-4">
                                <div>
                                    <div className="flex items-center gap-2 mb-2">
                                        <span className="badge-yellow flex items-center gap-1"><FiClock /> Pending</span>
                                        <span className="font-mono text-sm bg-slate-800 px-2 py-1 rounded">{req.jobId}</span>
                                    </div>
                                    ...
                                    {req.issueDescription && (
                                        <div className="mt-2 p-2 bg-slate-800 rounded-lg text-sm">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        onClick={() => setSelected({ id: req.id, caseId: req.caseId, action: 'approve' })}
                                        className="btn-success flex items-center gap-2"
                                    >
                                        <FiCheckCircle /> Approve
                                    </button>
                                    <button
                                        onClick={() => setSelected({ id: req.id, caseId: req.caseId, action: 'reject' })}
                ...
                <div className="fixed inset-0 bg-black/80 flex justify-center items-center z-50 p-4">
                    <div className="bg-slate-800 rounded-2xl max-w-md w-full p-6">
                        ...
                        <input
                            ...
                            className="input-field mb-3"
                            value={poDetails.unitPrice}
                            onChange={(e) => setPoDetails({ ...poDetails, unitPrice: parseFloat(e.target.value) })}
                        />
                        <input
                            type="number"
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => handleApprove(selected.id, selected.caseId)} className="btn-primary flex-1">Confirm</button>
            ...
            {selected && selected.action === 'reject' && (
                <div className="fixed inset-0 bg-black/80 flex justify-center items-center z-50 p-4">
                    <div className="bg-slate-800 rounded-2xl max-w-md w-full p-6">
                        ...
                        />
                        <div className="flex gap-3">
                            <button onClick={() => handleReject(selected.id, selected.caseId)} className="bg-red-600 text-white py-2 rounded-xl flex-1">Confirm Rejection</button>
                            <button onClick={() => setSelected(null)} className="btn-secondary flex-1">Cancel</button>
                        </div>
                    </div>
                </div>
