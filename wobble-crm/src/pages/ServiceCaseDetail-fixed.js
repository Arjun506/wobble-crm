  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'cases', id), {
        ...
        version: (caseData?.version || 0) + 1, // Version increment for concurrency
      });
      ...
      if (newStatus === 'In Progress' && previousStatus !== 'In Progress' && caseData) {
        try {
          ...
          console.error('Notification error:', notifErr); // Log only, no toast
        }
      }
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">Job ID: {caseData.jobId} | Version: {caseData.version || 0}</p>
      </div>
