...
import { FiTool, FiPlus, FiCheckCircle, FiCamera } from 'react-icons/fi';
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="card">
            <h3 className="text-xl font-semibold mb-4">Diagnosis & Status Update</h3>
            <textarea rows="4" className="input-field" value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)} placeholder="Enter diagnosis, repair notes..." />
            <select className="input-field mt-4" value={newStatus} onChange={(e) => setNewStatus(e.target.value)}>
              <option>Open</option>
        ...
        <div className="space-y-6">
          <div className="card">
            <h3 className="text-lg font-semibold mb-3 flex items-center gap-2"><FiTool /> Part Requests</h3>
            {partRequests.map((pr) => (
              <div key={pr.id} className="border border-slate-700 rounded-lg p-3 mb-2">
                ...
                {pr.status === 'Part Received' && <p className="text-green-400 text-xs mt-1 flex items-center gap-1"><FiCheckCircle /> Received</p>}
              </div>
            ))}
          ...
          <div className="card">
            <button onClick={handleCloseCase} className="w-full bg-red-600 hover:bg-red-700 text-white py-2 rounded-xl">Close Case</button>
          </div>
        </div>
      </div>
      ...
      {showPartForm && (
        <div className="fixed inset-0 bg-black/80 flex justify-center items-center z-50 p-4">
          <div className="bg-slate-800 rounded-2xl max-w-md w-full p-6">
            ...
            <textarea placeholder="Issue Description" className="input-field mb-3" rows="3" value={partForm.issueDescription} onChange={(e) => setPartForm({ ...partForm, issueDescription: e.target.value })} />
            <div className="flex gap-3"><button onClick={handleRaisePartRequest} className="btn-primary flex-1">Submit</button><button onClick={() => setShowPartForm(false)} className="btn-secondary flex-1">Cancel</button></div>
          </div>
        </div>
      )}
      ...
      {showPartReceived && (
        <div className="fixed inset-0 bg-black/80 flex justify-center items-center z-50 p-4">
          <div className="bg-slate-800 rounded-2xl max-w-md w-full p-6">
            ...
            <div className="mb-4"><label className="block text-slate-300 mb-2">Upload Photo of Received Part</label><input type="file" accept="image/*" onChange={(e) => setDamagedFile(e.target.files[0])} className="text-sm text-slate-400" /></div>
            <button onClick={() => handlePartReceived(showPartReceived.id, showPartReceived.partName)} disabled={uploading} className="btn-primary w-full mb-2">{uploading ? 'Uploading...' : 'Confirm Received'}</button>
